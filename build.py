...
try:
    ...
    vprint(f"Using ANDROID_HOME: {sdk_path}")
except KeyError:
    try:
        sdk_path = Path(os.environ["ANDROID_SDK_ROOT"])
        vprint(f"Using ANDROID_SDK_ROOT: {sdk_path}")
    except KeyError:
        # For CI environments, try to detect Android SDK location
        if "GITHUB_ACTIONS" in os.environ:
            vprint("Running in GitHub Actions, trying to detect Android SDK...")
            # Check common GitHub Actions environment variables
            github_sdk_paths = [
            ...
            github_sdk_paths = [p for p in github_sdk_paths if p is not None]
            
            for path_str in github_sdk_paths:
                path = Path(path_str)
                vprint(f"Checking path: {path}")
                if path.exists():
                    sdk_path = path
                    vprint(f"Found Android SDK at: {sdk_path}")
                    break
            else:
                # If no SDK found, create a minimal one for ndk command
                vprint("No Android SDK found, creating minimal one...")
                sdk_path = Path.cwd() / "android-sdk"
